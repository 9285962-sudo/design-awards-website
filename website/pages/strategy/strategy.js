...
// 筛选"参赛策略"分类的文章（排除三级栏目）
const strategyData = articlesData.articles
  .filter(a => a.category === '参赛策略' || a.id.includes('\\参赛策略\\') || a.id.includes('/参赛策略/'))
  .map(a => ({
    id: a.id,
    title: a.title,
    date: a.publishTime,
    category: a.category === '参赛策略' ? '参赛策略' : '参赛成长',
    excerpt: a.summary
  }))
...
export default function StrategyGuide() {
  return (
    <>
      <Head>
        <title>选奖指南 | 设计能</title>
        <meta name="description" content="如何选择最适合的国际设计大奖，选奖建议与推荐。" />
      </Head>
      ...
      <main>
        <section style={styles.hero}>
          <div style={styles.container}>
            <div style={styles.breadcrumb}><Link href="/strategy" style={styles.breadLink}>参赛策略</Link> / 参赛策略</div>
            <h1 style={styles.heroTitle}>参赛策略</h1>
            <p style={styles.heroDesc}>如何选择最适合的国际设计大奖</p>
          </div>
        </section>
        ...
        <section style={styles.section}>
          <div style={styles.container}>
            <div style={styles.categories}>
              ...
              <Link href="/strategy/strategy" style={styles.categoryActive}>参赛策略</Link>
